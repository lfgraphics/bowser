...
const TripVehicle = require('../models/tripVehicle');
...
router.get('/:vehicleNumber', async (req, res) => {
    ...
    try {
        const vehicles = await TripVehicle.find({ VehicleNo: { $regex: vehicleNumber, $options: 'i' } }).exec();
        ...
        res.status(200).json(vehiclesWithDrivers);
    } catch (err) {

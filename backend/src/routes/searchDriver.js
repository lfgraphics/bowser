...
router.get('/bowser-drivers/:parameter', async (req, res) => {
    ...
    try {
        const users = await findUsers({ $or: [{ userId: { $regex: parameter, $options: 'i' } }, { phoneNumber: { $regex: parameter, $options: "i" } }, { name: { $regex: parameter, $options: "i" } }] }, 'userId name phoneNumber roles verified');
...
router.post('/updateDriverMobile', async (req, res) => {
    ...
    try {
        ...
        const driver = await findOneDriver({ Name: { $regex: driverId, $options: 'i' } });

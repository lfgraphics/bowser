...
router.get('/orders/:id', async (req, res) => {
    try {
        ...
        const loadingOrder = await findLoadingOrderById(id).lean();
        if (!loadingOrder) {
        ...
        const bowser = await findOneBowser({ regNo: loadingOrder.regNo }).populate('currentTrip').lean();
...
router.post('/orders/get', async (req, res) => {
    try {
        ...
        const orders = await findLoadingOrders(filter).sort({ createdAt: -1 }).limit(20).lean();
...
router.post('/sheet', async (req, res) => {
    try {
        ...
        const resultSheet = await withTransaction(async (sessions) => {
            ...
            const bowser = await findOneBowser({ regNo }).session(sessions.bowsers);
            if (!bowser) {
...
router.post('/sheets/get', async (req, res) => {
    try {
        ...
        const sheets = await findLoadingSheets(filter).sort({ createdAt: -1 }).limit(20).lean();
...
router.get('/sheets/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const sheet = await findOneLoadingSheet({ _id: new mongoose.Types.ObjectId(id) }).lean();
        if (!sheet) {
...
router.patch('/sheets/:id', async (req, res) => {
    try {
        ...
        const updatedSheet = await withTransaction(async (sessions) => {
            const updated = await updateLoadingSheetById(
                new mongoose.Types.ObjectId(id),
                updateData,
                { new: true, runValidators: true, session: sessions.bowsers }
            );
...
router.delete('/sheet/:id', async (req, res) => {
    try {
        ...
        const deletedSheet = await withTransaction(async (sessions) => {
            const del = await deleteLoadingSheetById(new mongoose.Types.ObjectId(id), { session: sessions.bowsers });
            if (!del) {
                const e = new Error('LoadingSheet not found');
                e.name = 'ValidationError';
                throw e;

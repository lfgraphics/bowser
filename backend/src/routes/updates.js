...
const router = Router();
import Update, { find as findUpdates, aggregate as aggregateUpdates, findOne as findOneUpdate, deleteOne as deleteUpdate } from '../models/updates.js';

// Get all users with roles populated
router.get('/', async (req, res) => {
    const { appName = "drivers" } = req.query
    try {
        const updates = await findUpdates({ appName }).sort({ buildVersion: -1 }).lean();
...
router.get('/latest', async (req, res) => {
    try {
        const latestPerApp = await aggregateUpdates([
            { $sort: { buildVersion: -1 } },
            {
                $group: {
                    _id: "$appName",
...
router.get('/:id', async (req, res) => {
    try {
        const update = await findOneUpdate(req.params.id);

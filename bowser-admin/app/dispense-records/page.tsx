...
const VehicleDispensesPage = ({ searchParams }: { searchParams: { tripNumber?: number, allocator: string, limit?: number } }) => {
    ...
    const fetchRecords = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${BASE_URL}/listDispenses`, {
          params: params,
        });
        console.log(
          "records: ",
          response.data.records.map(
            (record: { dispenses: {} }) => record.dispenses
          )
        );
    ...
    const exportToExcel = async () => {
      ...
      try {
        ...
        const filterDescription = `${
          filter.bowserNumber ? `Bowser-${filter.bowserNumber} ,` : ""
        }${filter.driverName ? `Driver-${filter.driverName} ,` : ""}${
          filter.tripSheetId ? `Trip Sheet-${filter.tripSheetId} ,` : ""
        }`; //${selectedDateRange != undefined && dateDescription}`;
        const filename = `Dispenses_data ${filterDescription} ${
          records.length
        }record${
          records.length > 1 ? "s" : ""
        }, downloaded at ${timestamp}.xlsx`;

        const response = await axios.get(
          `${BASE_URL}/listDispenses/export/excel`,
          {
            params: {
              bowserNumber: filter.bowserNumber,
              driverName: filter.driverName,
    ...
    const toggleSelectAll = async () => {
      if (selectAll) {
        ...
        ); // Create a set of all row IDs
        setSelectedRows(allRowIds); // Select all
      }
      setSelectAll(!selectAll); // Toggle the select all state
    };

    const verifyOne = async (id: string, tripSheetId: string) => {
      try {
        setLoading(true);
        let response = await axios.patch(
          `${BASE_URL}/listDispenses/verify/${id}`,
          { by: { id: user?.userId, name: user?.name }, tripSheetId }
        );
        if (response.status == 200) {
          setRecords((prevRecords) =>
            prevRecords.map((record) =>
              record._id === id
                ? { ...record, verified: { status: true } }
                : record
            )
          );
          toast({
    ...
    const verifyMultiple = async () => {
      ...
      try {
        let response = await axios.post(`${BASE_URL}/listDispenses/verify/`, {
          ids: idsToVerify,
          by: { id: user?.userId, name: user?.name },
        });
        if (response.status === 200) {
          setRecords((prevRecords) =>
            prevRecords.map((record) =>
              idsToVerify.includes(record._id)
                ? { ...record, verified: { status: true } }
                : record
            )
          );
            ...
            {records?.length > 0 &&
              records.map((record, index) => (
                ...
                >
                  ...
                  <TableCell>
                    {record?.fuelingDateTime?.length > 1
                      ? `${formatDate(record.fuelingDateTime)}`
                      : ""}
                  </TableCell>
                  <TableCell>{record?.bowser?.regNo}</TableCell>
                  <TableCell>
                    {record?.gpsLocation !== undefined &&
                      record.gpsLocation?.substring(0, 15) + "..."}
                  </TableCell>
                  ...
                  <TableCell>
                    {record?.verified ? (
                      ...
                      <Button
                        onClick={() => {
                          verifyOne(String(record?._id), record.tripSheetId);
                        }}
                        variant={
                          selectedRows.has(`${record?._id}`)
                            ? "secondary"
